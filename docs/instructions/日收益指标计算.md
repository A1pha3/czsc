...
**版本**: v1.1.0
**更新日期**: 2026-02-26
**模块**: `czsc.utils.analysis`

## 概述

日收益指标计算是量化投资中评估策略表现的重要工具。本文档基于 `czsc.utils.analysis.stats` 模块，详细说明各项指标的计算方法和实际应用。

> **注意**: `daily_performance` 函数现在位于 `czsc` 主模块中（Rust 实现），同时也提供 Python 版本在 `czsc.utils.analysis.stats` 中作为回退方案。
...
### 统计分析模块 (czsc.utils.analysis)

```python
# 推荐使用方式
from czsc import daily_performance, top_drawdowns  # Rust 版本（默认）
from czsc.utils.analysis import (
    daily_performance,           # Python 版本（回退方案）
    rolling_daily_performance,    # 滚动计算
    holds_performance,            # 持仓绩效
    psi,                         # 群体稳定性指标
)
...
| 函数 | 位置 | 说明 |
|------|------|------|
| `daily_performance` | `czsc` / `czsc.utils.analysis.stats` | 日绩效计算 |
| `top_drawdowns` | `czsc` / `czsc.utils.analysis.stats` | 最大回撤计算 |
| `rolling_daily_performance` | `czsc.utils.analysis.stats` | 滚动日绩效 |
| `holds_performance` | `czsc.utils.analysis.stats` | 持仓绩效计算 |
| `cal_break_even_point` | `czsc.utils.analysis.stats` | 盈亏平衡点计算 |

### 相关模块

- **回测可视化**: `czsc.utils.plotting.backtest`
- **回测分析**: `czsc.traders.weight_backtest`
- **权重分析**: `czsc.utils.plotting.weight`
